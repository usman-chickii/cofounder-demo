...
const STAGE_KEY: ChatbotStage = "final_summary";
const PRELOADED_PROMPT = getSystemPrompt(STAGE_KEY);

interface FinalSummaryInput {
  projectId: string;
  userMessage: string;
  onData: (chunk: string) => void;
}

export const finalSummaryNode = new RunnableLambda<
  FinalSummaryInput,
  { stage: ChatbotStage }
>({
  func: async ({ projectId, userMessage, onData }: FinalSummaryInput) => {
